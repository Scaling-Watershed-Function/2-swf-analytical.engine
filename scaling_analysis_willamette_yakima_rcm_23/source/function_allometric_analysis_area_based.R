...
############ TEST
allometric_analysis <- function(data, x_col, y_col, n, best_iterations = FALSE) {
  ...
  identify_best_iterations <- function(regression_df) {
    regression_df <- regression_df %>%
      arrange(desc(R_Squared), SSR)
    
    top_r2 <- regression_df$R_Squared[1]
    min_ssr_for_top_r2 <- regression_df$SSR[which.max(regression_df$R_Squared)]
    labeled_df <- regression_df %>%
      mutate(Label = ifelse(R_Squared == top_r2 & SSR == min_ssr_for_top_r2, 
                            "meet_r2_ssr_req", 
                            "do_not_meet_r2_ssr_req"))
...
# Create an empty list to store results_df and regression results from each iteration
  ...
  for (i in 1:n) {
    ...
    for (y_col_name in y_cols) {
      ...
      slope <- coef(model)[2]
      r_squared <- summary(model)$r.squared
      residuals <- log(response) - predict(model)
      ssr <- sum(residuals^2)
      regression_results[[paste("iteration", i, y_col_name)]] <- data.frame(
        Intercept = intercept,
        Slope = slope,
...
# If best_iterations is TRUE, label the iterations
if(best_iterations) {
  regression_df <- identify_best_iterations(regression_df)
}
...
summary_df <- data.frame(
  ...
  Avg_SSR = c(mean(regression_df$SSR[grep("_max", rownames(regression_df))]),
               mean(regression_df$SSR[grep("_med", rownames(regression_df))]),
               mean(regression_df$SSR[grep("_min", rownames(regression_df))])),
  SSR_Lower_CI = c(quantile(regression_df$SSR[grep("_max", rownames(regression_df))], 0.025),
                    quantile(regression_df$SSR[grep("_med", rownames(regression_df))], 0.025),
                    quantile(regression_df$SSR[grep("_min", rownames(regression_df))], 0.025)),
  SSR_Upper_CI = c(quantile(regression_df$SSR[grep("_max", rownames(regression_df))], 0.975),
                    quantile(regression_df$SSR[grep("_med", rownames(regression_df))], 0.975),
                    quantile(regression_df$SSR[grep("_min", rownames(regression_df))], 0.975))
)
if (best_iterations) {
  labeled_regression_df <- identify_best_iterations(regression_df)
  return(list(Summary = summary_df, Regression_Data = labeled_regression_df))
} else {
  return(summary_df)
}
