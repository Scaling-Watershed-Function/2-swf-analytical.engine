...
scaling_plot <- generate_plot(scaling_map_dat, "scaling_cat", scale_color_viridis_d, "Scaling type", "Scaling", TRUE)
precipt_plot <- generate_plot(scaling_map_dat, "log_mean_ann_pcpt_mm", scale_color_viridis_c, "Precipitation \nlog[mm]", "Precipitation")
elevat_plot <- generate_plot(scaling_map_dat, "wshd_avg_elevation_m", scale_color_gradientn, "Elevation (m)", "Elevation", colours = terrain.colors(10))
heterog_plot <- generate_plot(scaling_map_dat, "hrel_3", scale_color_viridis_c, "Entropy \n(relative)", "Landscape entropy", option = "mako")
