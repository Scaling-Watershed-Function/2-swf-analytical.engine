...
p <- ggplot(data = scaling_resp_dat,
            aes(x = wshd_area_km2,
...
sa_exp_mod <- lm(log(accm_stream_area_m2)~log(wshd_area_km2),
                 data = filter(scaling_resp_dat,basin == "yakima"))
confint(sa_exp_mod)
...
d50_mod <- lm(log(d50_m)~(log(bnkfll_width_m)+log(reach_slope)+
           log(mean_ann_flow_m3s)+log(wshd_area_km2))*basin+stream_order,
              data = scaling_resp_dat,
              na.action = na.omit)

summary(d50_mod)

# Let's check model performance excluding default values for reach slope (>0.000001)

d50_mod_i <- lm(log(d50_m)~(log(bnkfll_width_m)+log(reach_slope)+
             log(mean_ann_flow_m3s))*basin+stream_order,
              data = filter(scaling_resp_dat, d50_m > 1e-05),
              na.action = na.omit)
